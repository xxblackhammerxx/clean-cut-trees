...
// API credentials
const API_USERNAME = 'eric@gainzmarketing.com'
const API_PASSWORD = 'brothers4eva'
const API_BASE_URL = 'http://localhost:3000/api' // Adjust if different
...
async function authenticateWithPayload(): Promise<string> {
  try {
    console.log('🔐 Authenticating with Payload CMS API...')
...
async function fetchAllPages(token: string): Promise<PayloadPage[]> {
  try {
    console.log('📡 Fetching pages from Payload CMS API...')

    let allPages: PayloadPage[] = []
    let page = 1
    let hasMore = true

    while (hasMore) {
      const response = await fetch(
        `${API_BASE_URL}/pages?limit=50&page=${page}&where[status][equals]=published`,
        {
          method: 'GET',
          headers: {
      ...
      allPages = [...allPages, ...data.docs]

      console.log(`   📄 Fetched page ${page}/${data.totalPages} (${data.docs.length} pages)`)
...
const generatePageComponent = (page: PayloadPage): string => {
  const escapeForJSX = (text: string): string => {
    return text
      .replace(/\\/g, '\\\\')
      .replace(/`/g, '\\`')
      .replace(/\$/g, '\\$')
      ...
      switch (node.type) {
          ...
          const paragraphContent = children
            .map((child: LexicalNode) => {
              if (child.type === 'text') {
                ...
                let text = escapeForJSX(textNode.text || '')
              ...
              if (child.type === 'link') {
                ...
                const linkText = linkChildren
                  .map((c: LexicalTextNode) => escapeForJSX(c.text || ''))
                  .join('')
                return `<Link href="${escapeForJSX(linkNode.url || '#')}">${linkText}</Link>`
              }
              return ''
            })
            .join('')
        ...
        case 'heading':
          ...
          const headingText = headingChildren
            .map((child: LexicalTextNode) => escapeForJSX(child.text || ''))
            .join('')
...
${listItems}
        ...
        case 'image':
          ...
          const imageUrl = escapeForJSX(imageNode.src || imageNode.value?.url || '')
          const imageAlt = escapeForJSX(imageNode.alt || imageNode.value?.alt || '')
...
${renderedNodes}
  ...
  const renderMarkdownContentToJSX = (content: string): string => {
    const escapedContent = escapeForJSX(content)
    return `        <div 
          className="markdown-content" 
          dangerouslySetInnerHTML={{ 
            __html: marked(\`${escapedContent}\`) 
          }} 
        />`
  }
  ...
  const seoTitle = escapeForJSX(page.seo?.title || page.title)
  const seoDescription = escapeForJSX(
    page.seo?.description || page.excerpt || `Learn about ${page.title} at Clean Cuts Trees`,
  )
  const seoKeywords = escapeForJSX(page.seo?.keywords || 'tree service, Clean Cuts Trees, Utah')
  const pageTitle = escapeForJSX(page.title)

  const needsMarked = typeof page.content === 'string'
  const needsLink =
    typeof page.content === 'object' && JSON.stringify(page.content).includes('"type":"link"')

  // Create a safe component name from the slug
  const componentName =
    page.slug
      .split('-')
              ...
              {isServiceAreaPage && (
                <div className="service-area-cta">
                  <div className="cta-box">
                    <h3>
                      Serving{' '}
                      {'${page.title.replace('Tree Service ', '').replace(', UT - Clean Cuts Trees', '')}'}
                    </h3>
...
async function convertPagesToStatic() {
  console.log('🚀 Starting conversion of Payload CMS pages to static files via API...')
